...
scheduler.attachEvent("onClick", function(id, e){
	//click on event title open the details instead of quick info
	if (e && e.target && e.target.nodeName.toLowerCase() == 'span'){ 
...
scheduler._render_v_bar = function (id, x, y, w, h, style, contentA, contentB, bottom) {
	...
	if (scheduler.renderEvent && !bottom && scheduler.renderEvent(container, ev)) {
		...
		inner_html += '<div class="dhx_body" style=" width:' + (w - (this._quirks ? 4 : 14)) + 'px; height:' + (h - (this._quirks ? 20 : 30) + 1) + 'px;' + bg_color + '' + color + '"><span>' + contentB + '<span></div>'; // +2 css specific, moved from render_event
