...
// From mootools b1.2 trunk, for the new Selectors implementation
Native.UID = 1;
...
Element.implement({
	...
	appendText: function(text, where){
		// added by james - this is the new code for appending text but it breaks in FF
		if (window.ie) { return this.grab(this.getDocument().newTextNode(text), where) };
	...
	*/
	...
	clone: function(contents, keepid){
		switch ($type(this)){
			...
			//case 'whitespace': return document.createTextNode('\u00A0');  // Try to fake it by using &nbsp;
		}
		return null;
	},
...
Element.implement({
	...
	// This is the current getPosition in mootools, but using it breaks the itinerary tool
	getRealPosition: function(relative) {
...
// 		return {x: position.x - relativePosition.x, y: position.y - relativePosition.y};
	...
	getRelativePosition: function(){
		return this.getPosition(this.offsetParent);
	},
	...
	getRealCoordinates: function(element){
		if (isBody(this)) return this.getWindow().getCoordinates();
		var position = this.getRealPosition(element), size = this.getSize();
		var obj = {left: position.x, top: position.y, width: size.x, height: size.y};
		obj.right = obj.left + obj.width;
		obj.bottom = obj.top + obj.height;
		return obj;
...
Note:
	...
	This is a modified version of the original mootools slider that includes the ability to add a range area as 
	well as second knob.  I also added a new feature that will report back units while retaining the full number of steps.
*/
var Slider = new Class({
	...
	options: { /*
		onChange: $empty,
		onComplete: $empty, */
		onTick: function(pos, type){
			var knobType = type ? 'maxknob' : 'knob';
			this[knobType].setStyle(this.property, pos);			
		},
		range: false,
		offset: 0,
		knobheight: 18,
  ...
  initialize: function(el, knob, options) {
		...
		// Added
		this.maxknob = this.options.maxknob;
		this.rangearea = this.options.rangearea;
		this.unitSize = this.options.steps / this.options.units;
		
		// If there is no max knob
		if (!this.maxknob) this.element.addEvent('mousedown', this.clickedElement.bindWithEvent(this));
		var mod, offset;
		...
		// For some reason, the this.full calculation leaves off a few pixels in some case,
		// making it impossible to reach the max value.  We introduce a small delta to create
		// some leeway
		this.delta = 3;
		
		var lim = {};
		lim[this.axis] = [- this.options.offset, this.max - this.options.offset];
		this.drag = new Drag(this.knob, {
			limit: lim,
			modifiers: mod,
			snap: 0,
			// We need the draggedKnob to be fired without any arguments
		...
		if(this.maxknob) {  
			...
			this.maxknob.setStyle('position', 'absolute').setStyle(this.property, - this.options.offset);
			this.maxdrag = new Drag(this.maxknob, {
				limit: lim,
				modifiers: mod,
				snap: 0, 
				onStart: this.draggedKnob.bind(this, 1),
				onDrag:  this.draggedKnob.bind(this, 1),
				onComplete: function(){
					this.draggedKnob(1);
					this.end();
				}.bind(this)
		...
		if (this.options.snap) {
			...
			this.drag.options.limit[this.axis][1] = this.full + this.delta;
			if (this.maxdrag) {
				this.maxdrag.options.grid = Math.ceil(this.stepWidth);
				this.maxdrag.options.limit[this.axis][1] = this.full + this.delta;
			}
		}
	},

	set: function(step, type){
		var stepType = 'step';
		...
		this[stepType] = Math.round(step);
		this.checkStep(type);
		this.end();
		this.fireEvent('onTick', [this.toPosition(this[stepType]), type]);
		return this;
	},
	...
	draggedKnob: function(max) {
		var prefix = max ? 'max' : '';
		this[prefix + 'step'] = this.toStep(this[prefix + 'drag'].value.now[this.axis]);
		this.checkStep(max);
	},

	checkStep: function(max){
		var minstep;
		var maxstep;
		var minunit;
		var maxunit;
		...
		this.stepUnit = Math.ceil(this.step / this.unitSize);
		if (this.maxknob) {
			this.maxstepUnit = Math.ceil(this.maxstep / this.unitSize);
		} else {
			// Prepare the range area
			minstep = 0;
			maxstep = this.step;
		}
		
		if(!max) {
			if (this.previousChange != this.stepUnit){
				this.previousChange = this.stepUnit;
				changed = true;
			}
		} else if (this.maxPreviousChange != this.maxstepUnit){
			this.maxPreviousChange = this.maxstepUnit;
			changed = true;
		}
		
		// Update the maxknob and swap it with the min knob if we have to
		if(this.maxknob) {
			minstep = (this.step < this.maxstep) ? this.step : this.maxstep;
			maxstep = (this.step < this.maxstep) ? this.maxstep: this.step;
		
			// Showing units
			minunit = (this.options.showunits) ? Math.ceil(minstep / this.unitSize) : minstep;
			maxunit = (this.options.showunits) ? Math.ceil(maxstep / this.unitSize) : maxstep;
			if (changed) this.fireEvent('onChange', { min: minunit, max: maxunit });
		} else {
			var unit = this.options.showunits ? Math.ceil(this.step / this.unitSize) : this.step;
			if (changed) this.fireEvent('onChange', unit + '');
		}
		
		// Update the range area
		if (this.rangearea) {
			var minpos = this.toPosition(minstep);
			var maxpos = this.toPosition(maxstep);
			this.rangearea.setStyles({ 
				width: maxpos - minpos + this.delta,
				left: minpos
			});
		}
	},
		...
		if (this.previousEnd != this.step || (this.maxknob && this.maxPreviousEnd != this.maxstep)) {
			...
			if(this.maxknob) {
				...
				maxstep = (this.step < this.maxstep) ? this.maxstep: this.step;
				if (this.showunits) {
					minstep = Math.ceil(minstep / this.unitSize);
					maxstep = Math.ceil(maxstep / this.unitSize);
				}
				this.fireEvent('onComplete', { min : minstep + '', max: maxstep + '' });
			} else {  
				var unit = this.options.showunits ? Math.ceil(this.step / this.unitSize) : this.step;
				this.fireEvent('onComplete', unit + '');
			}
		}
	},
...
Element.implement({
	show: function(display) {
		if (display == 'visible') this.style.visibility = 'visible';
		else this.style.display = (typeof(display) == 'string') ? display : 'block';
		return this;
	},
	hide: function(display) {
		if (display == 'hidden') this.style.visibility = 'hidden';
...
/**
 * Class: OverlayFix
 * This was taken from digitarald's auto complete script.  Adds the iframe shim to prevent IE select box/flash bleed through
 */
var OverlayFix = new Class({
	...
	initialize: function(el) {
		...
		if (window.ie){
			...
			var frameSource = (loc.match(/^https/)) ? '/assets/blank.html' : 'javascript:false';
			...
			this.fix = new IFrame({
				...
				styles: { 
					...
					border: 'none',
					display: 'none',
					filter: 'progid:DXImageTransform.Microsoft.Alpha(opacity=0)'+ 'px', zIndex: 999 
				}
